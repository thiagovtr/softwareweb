## Trabalho Final de Engenharia de Software
## Desenvolvemente uma aplicação Web
Trabalho Final em grupo para a Disciplina de GCC188 - Engenharia de Software no Curso de Sistemas de Informação na Universidade Federal de Lavras (UFLA).

## 1. Identificação do projeto
Nome do projeto: [Preencher]
Problema escolhido: [Informar o problema apresentado escolhido e validado pelo professor ]
Turma/Semestre: 2026/01
Docente: Johnatan Oliveira

## Integrantes do grupo
Bruno Santos Vilas Boas — função principal no grupo
Christian Silva Mesquita — função principal no grupo
Guilherme dos Santos Fernandes — função principal no grupo
Matheus Levi Tavares — função principal no grupo 
Thiago Vinícius Tristão Rojas — função principal no grupo 

## Sprint atual
Sprint 1 – Definição do problema e visão do produto